...
function chk_root () {
	if [[ "$(id -u)" -ne 0 ]]; then
		banner
		printf " ${BD}${WH}[${RD}!${WH}] ${RD}Knashi Deauther must be run as root ${YW}｡ﾟ･（>﹏<）･ﾟ｡\n"
		exit 1
	fi
}
...
function banner () {
	printf "${BD}
${RD}© Smert - 2023
 ${NT}${WH}[${RD}!${WH}] I am not responsible for damage caused by ${BD}${GR}Knashi Deauther${WH}${NT}.
...
function get_interface () {
	...
	echo -ne "\n${RD}Smert${GR}@${RD}Kaonashi: ${WH}>> "; read iface
...
function deactivate_destruction () {
	...
	clear
	banner
	printf " ${BD}${WH}[${RD}*${WH}] ${RD}Deactivate Destructive Packet Injection ...\n"
	printf " ${BD}${WH}[${RD}*${WH}] ${RD}頭いい Thank you ... ${GR}~(=^‥^)丿\n"
	rm -f .iface.tmp
	exit
}
...
function deactivate_destruction_2 () {
	...
	clear
	banner
	printf " ${BD}${WH}[${RD}*${WH}] ${RD}Deactivate Destrutive Packet Injection ...\n"
	printf " ${BD}${WH}[${RD}*${WH}] ${RD}頭いい Thank you ... ${GR}~(=^‥^)丿\n"
	rm -f .iface.tmp
...
printf " ${WH}4) ${GR}Exit\n"
echo -ne "\n${RD}Smert${GR}@${RD}Kaonashi: ${WH}>> "; read attack
clear
...
if [[ $attack == 1 ]]; then
	...
	printf "${NT}\n"
	nmcli dev wifi
	echo -ne "\n${RD}Smert${GR}@${RD}Knashi: ${WH}>> "; read attck_ssid
	clear
	banner
	get_interface
	clear
	banner_2
	printf "                         ${WH}[ ${GR}Knashi Deauther ${WH}]\n"
	printf "              ${WH}===== ${RD}頭いい Begun To Deauthenticate Clients 頭いい ${WH}=====\n\n"
	...
	printf "${NT}\n"
	nmcli dev wifi
	echo -ne "\n${RD}Smert${GR}@${RD}Kaonashi: ${WH}>> "; read attck_chnl
	clear
	banner
	get_interface
	clear
	...
	echo -ne "\n${RD}Smert${GR}@${RD}Kaonashi: ${WH}>> "; read spm
	if [[ $spm == 1 ]]; then
		...
		clear
		banner
		printf "\n${RD}Smert${GR}@${RD}Kaonashi${WH}(SSID(Name of Network)) >> "; read rand_ssid;
		printf "\n${RD}Smert${GR}@${RD}Kaonashi${WH}(How Many SSID) >> "; read con_ssid;
